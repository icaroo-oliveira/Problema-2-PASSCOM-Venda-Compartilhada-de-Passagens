...
ARQUIVO_GRAFO = 'grafo_C.json'
ARQUIVO_PASSAGENS_COMPRADAS = 'passagens_C.json'
ARQUIVO_ROLLBACKS_FALHOS = 'rollbacks_falhos_C.json'
...
def encontrar_caminhos(cidade_inicial, cidade_fim):
    ...
    # Se houver pelo menos um caminho válido, extrai os caminhos e insere "C" no início. Não precisa mais da distância
    if caminhos:
        caminhos_ordenados = ["C"]  # Esse "C" vai indicar que os caminhos são do servidor C
...
def verifica_compras_cpf(cpf):
    ...
    if cpf in compras:
        ...
        # Indica que é do server C
        compras_cliente.insert(0, "C")
...
# Função para registrar que servidor A ou B não realizou um rollback
