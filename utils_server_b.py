...
ARQUIVO_GRAFO = 'grafo_B.json'
ARQUIVO_PASSAGENS_COMPRADAS = 'passagens_B.json'
ARQUIVO_ROLLBACKS_FALHOS = 'rollbacks_falhos_B.json'
...
def encontrar_caminhos(cidade_inicial, cidade_fim):
    ...
    # Se houver pelo menos um caminho válido, extrai os caminhos e insere "B" no início. Não precisa mais da distância
    if caminhos:
        caminhos_ordenados = ["B"]  # Esse "B" vai indicar que os caminhos são do servidor B
...
def verifica_compras_cpf(cpf):
    ...
    if cpf in compras:
        ...
        # Indica que é do server B
        compras_cliente.insert(0, "B")
...
# Função para registrar que servidor A ou C não realizou um rollback
